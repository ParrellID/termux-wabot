...
let { conn, args } = data
try {
        if (!args[0]) return m.reply('[❓] Masukkan Query dengan Angka!')
       if (!args[1]) return m.reply('[❓] Masukkan Query dengan Angka lagi!')
        ...
        } else m.reply(`[❗] Reply / Balas VN dengan Caption *${userbot.prefix}bass*`)
    } catch (e) {
        throw e
    }
}
