...
module.exports = {
  ...
  if (!/image/.test(mime)) throw `❗| Kirim Gambar / Reply Gambarnya`
  try { q = m.quoted.download() }
  catch (e) { q = m.download() }
  m.reply('_⏳ | Sedang membuat..._')
  running(await q, d).then(vid => conn.sendFile(m.chat, vid, 'run.mp4', 'FarrellID Development', m))
  }
}
