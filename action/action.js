...
console.log("[BOT-LOG] Joining Members " + participants[0].split("@")[0])
...
teks = `User @${mem.split("@")[0]} has left The Group ${groupM.subject}`
 conn.sendButtonLoc(jid, buff, teks, userbot.packname, 'GOODBYE FRIEND', 'GOODBYE', false, { contextInfo: {"mentionedJid": conn.parseMention(teks)}})
    }
...
let imaged = await new Canvas.Welcome()
 ...
 teks = `User @${mem.split("@")[0]} has been Joined Group ${groupM.subject}`
 conn.sendButtonLoc(jid, buff, teks, userbot.packname, 'WELCOME FRIEND', 'WELCOME', false, { contextInfo: {"mentionedJid": conn.parseMention(teks)}})
    }
 break
}
}
