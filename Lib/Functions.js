...
console.log(this.chalk.blueBright.italic('[LOGIN-LOGS] Login Information Updated!'))
});
conn.on('open', async () => { 
console.log(this.chalk.green.bold('[LOGIN-LOGS] Succesfully Login to Whatsapp'))
}); 
...
console.log(`[ UPDATE-LOGS] File '${filename}' is Update`);
if (this['fs'].existsSync(dir));
else {
conn.logger.warn(`[DELETED-LOGS] Deleted Plugin '${filename}'`);
return delete global.plugins[filename];
}
} else conn.logger.info(`[ADD-LOGS] Adding Plugins '${filename}'`);
var err = this.syntaxerror(this['fs'].readFileSync(dir), filename);
if (err) conn.logger.error(`[ERROR-LOGS] Syntax Error While Loading '${filename}'\n${err}`);
else try {
...
} else conn.logger.info(`[REQUEST-LOGS] Requiring New Plugin '${filename}'`);
var err = this.syntaxerror(this['fs'].readFileSync(dir), filename);
if (err) conn.logger.error(`[ERROR-LOGS] Syntax Error While Loading '${filename}'\n${err}`);
else try {
