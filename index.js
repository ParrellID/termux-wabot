...
let [ffmpeg, ffprobe, ffmpegWebp, convert, magick, gm] = test
  ...
  if (!s.ffmpeg) conn.logger.warn('FFMPEG ERROR! Install FFMPEG for Continued')
  if (s.ffmpeg && !s.ffmpegWebp) conn.logger.warn('LIBWEBP ERROR! Install LIBWEBP for Continued')
  if (!s.convert && !s.magick && !s.gm) conn.logger.warn('IMAGEMAGIC ERROR! Install IMAGEMAGIC for Continued')
}

_quickTest()
  .then(() => conn.logger.info('[SUCCES-INFO] Quick Test Done'))
  .catch(console.error)

conn.version = [ 2, 2140, 12 ]
conn.logger.level = "warn"
conn.browserDescription = ['FarrellID', 'SAFARI', '8.1']
...
setInterval(() => {
 conn.setStatus(`Build by Farrell Athar Ramadhan | Prefix: ${userbot.prefix} | UpTime: ${Ft.count(process.uptime())} | Listening Spotify`).catch((_) => _)
},1000)
 require('./src/loader');
